...
**One-liner:** A Meteora DBC launch primitive for tokenized stocks: pools quoted in the stock token itself, curves anchored to a median-of-sources reference, atomic launches with no sniper window, and a keeper that runs the lifecycle. Thirteen pools on mainnet, three fee profiles with every stream claimed on-chain, a shared-config launchpad, two Clawpump agent tokens paired with Backpack/Sunrise stocks (SPCX, SNDK) and pooled on Meteora.
...
**Why now:** Tokenized stocks on Solana did $200M+ in a day on Sept 12, 63% of it after market hours (Solana Compass). Meteora announced "pair anything with Backpack-issued stocks" on Sept 15; all 48 issued Backpack mints carry a Meteora DBC token badge, which is the plumbing this project runs on. Backpack has 1,158 pre-deployed stock mints and 48 issued: the other 1,110 have no price-discovery path, which is what a stock-quoted launch curve is for.

**Revenue proof:** the seed-profile pool sSNDK/SOL had its 0.9 SOL raise filled by a third party within seconds of the atomic launch; we withdrew 0.79 SOL of partner LP on-chain afterwards and kept the 10% locked share. Issuer-profile pools claimed all three fee streams (listing 3%, creation, trading) on-chain. At a $50k raise the same profile books ~$1.5k on day one (`size.mjs`).
...
- **Stocknized Agent on Clawpump** — $SOLENT/SPCX launched with Clawpump's own Meteora DBC launcher (pool AaPSRyVpG2c2BWrVHDDSX9kWBvVRdozfxzA5RmUAPzjD, fees in SPCX), plus three agent tokens launched through Clawpump's API (scSPCX/SPCX, scSNDK/SNDK, BonkClaw/CLAW), run by the Clawpump agent Solent, whose wallet is SAID-verified on-chain (597yKNtaZBasmu61ootY5pBWHfktKXzFP38fB7K3A6g6); two, each paired with a Backpack/Sunrise stock and pooled on Meteora: scSPCX/SPCX (SpaceX, Sunrise's flagship) and scSNDK/SNDK. https://solscan.io/account/GYN1e3YjRrWuAZDVJthgW1wYJBPaciZxmW1gfDRaFkHP
